...
def create_training_config(
    ...
    hsv_h=0.1,    # Hue adjustment - 색조 변경 (범위: 0.0-1.0)
    hsv_s=0.7,      # Saturation adjustment - 채도 변경 (범위: 0.0-1.0)
    hsv_v=0.7,      # Brightness adjustment - 밝기 변경 (범위: 0.0-1.0)
    #single_cls=True,
    lr0=1E-2,
    ...
    **kwargs
    ...
    Args:
        ...
        lrf: 최종 학습률 비율
        hsv_h: 색조 조정 - 조명 변화에 강건한 모델 훈련 (0.0-1.0)
        hsv_s: 채도 조정 - 다양한 날씨/카메라 조건 대응 (0.0-1.0)
        hsv_v: 밝기 조정 - 다양한 조명 조건 대응 (0.0-1.0)
    """
    ...
    config = {
        ...
        # Geometric Augmentations
        'flipud': flipud,     # 상하 반전 확률
        'mixup': mixup,       # 이미지 블렌딩 확률
        ...
        # Color Space Augmentations (HSV)
        'hsv_h': hsv_h,       # 색조 변화 범위 (±hsv_h)
        'hsv_s': hsv_s,       # 채도 변화 범위 (±hsv_s)
        'hsv_v': hsv_v,       # 밝기 변화 범위 (±hsv_v)
...
def main():
    ...
    # add augmentation
    parser.add_argument('--translate', type=float, default=0.4, help='translate')
    parser.add_argument('--sclae', type=float, default=0.4, help='scale')
    parser.add_argument('--shear', type=float, default=10.0, help='shear')
    parser.add_argument('--perspective', type=float, default=0.001, help='perspective')
    parser.add_argument('--degrees', type=float, default=90.0, help='degrees')

    # Color Space Augmentation 설정
    parser.add_argument('--hsv_h', type=float, default=0.1, help='색조 변화 범위 (0.0-1.0)')
    parser.add_argument('--hsv_s', type=float, default=0.7, help='채도 변화 범위 (0.0-1.0)')
    parser.add_argument('--hsv_v', type=float, default=0.7, help='밝기 변화 범위 (0.0-1.0)')
    
    # 기타 Augmentation 설정
    parser.add_argument('--flipud', type=float, default=0.7, help='상하 반전 확률 (0.0-1.0)')
    parser.add_argument('--mixup', type=float, default=0.7, help='이미지 블렌딩 확률 (0.0-1.0)')
    ...
    try:
        ...
        logger.info("=== Color Space Augmentations ===")
        logger.info(f"hsv_h (색조): {args.hsv_h}")
        logger.info(f"hsv_s (채도): {args.hsv_s}")
        logger.info(f"hsv_v (밝기): {args.hsv_v}")
        logger.info("=== 기타 Augmentations ===")
